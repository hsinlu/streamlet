    ...
    <div class="row">
        <div class="col-md-9">
            <div class="form-group">
                ...
                value="{{ old('title') }}" placeholder="{{ trans('strings.admin.article_edit.title_placeholder') }}" required/>
        ...
        <div class="col-md-3">
            ...
            <div class="form-group">
                ...
                <input id="slug" type="text" class="form-control" name="slug" placeholder="Article Url" v-model="slug" value="{{ old('slug') }}"/>
                <p class="help-block">{{ url('/') }}/@{{slug}}</p>
            </div>

            <div class="form-group">
                <label for="category">{{ trans('strings.admin.article_edit.article_category') }}</label>
                <select name="category" id="category" class="form-control" value="{{ old('category') }}" multiple required>
                    @foreach($categories as $category)
                        <option value="{{ $category->name }}" {{ old('category') == $category->name ? 'selected="selected"' : '' }}>{{ $category->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="tags[]" id="tags" class="form-control" value="{{ old('tags') }}" multiple>
                    @foreach($tags as $tag)
                        <option value="{{ $tag->name }}" {{ !is_null(old('tags')) && in_array($tag->name, old('tags')) ? 'selected="selected"' : '' }}>{{ $tag->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="checkbox">
                <label>
                    <input name="public" type="checkbox" {{ old('public') ? 'checked' : '' }}>{{ trans('strings.admin.article_edit.public') }}
                </label>
            </div>
